...
Status: executable contract harness plus a representative Phase 0 consumer. `graph-projection.schema.json` and the fixture corpus define the versioned server response contract shared by Explore Map and Outline. Strict Ajv structural validation and deterministic semantic validation live in `src/shared/contracts/graph-projection.ts`; the contract tests include a complete deterministic 500-node stress materializer. The `/explore` slice adds a read-only validated composer, deterministic Dagre adapter, React Flow Map, contract-Outline view, and measured 25-node Node/browser gates. It deliberately reads the representative fixture, not production database state. A strict authenticated `ExploreSourceV1` database adapter now supplies only the Catalog/Targets/Overlay portion of future composition; the complete source-to-projection materializer and inspector-detail fetching remain future work.
