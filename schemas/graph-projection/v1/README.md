...
Status: executable contract harness. `graph-projection.schema.json` and the fixture corpus define the versioned server response contract shared by Explore Map and Outline. Strict Ajv structural validation and deterministic semantic validation live in `src/shared/contracts/graph-projection.ts`; the contract tests include a complete deterministic 500-node stress materializer. A production projection serializer, React Flow adapter, Dagre runtime, and measured browser p95 harness remain outside this gate.
