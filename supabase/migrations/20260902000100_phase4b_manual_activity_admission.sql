-- Phase 4B: exact-preview admission of one accepted personal activity to the initial Track.
...
-- Overlay owns eligibility and returns a bounded internal page. Planning supplies only the exact
-- profile and its server-derived set of already-attributed activity UUIDs.
create function overlay.get_planning_activity_admission_choices_v1(
  p_workspace_id uuid,
  p_profile_version_id uuid,
  p_excluded_custom_activity_ids uuid[]
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if p_workspace_id is null or not identity.is_workspace_member(p_workspace_id)
     or p_profile_version_id is null
     or p_excluded_custom_activity_ids is null
     or pg_catalog.array_position(p_excluded_custom_activity_ids, null) is not null then
    raise exception using errcode = '42501', message = 'activity admission choices are unavailable';
  end if;

  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(p_workspace_id::text || ':overlay.workspace', 0)
  );
  select root.aggregate_version into v_overlay_version
  ...
  where activity.workspace_id = p_workspace_id
    ...
    and activity.custom_activity_id <> all(p_excluded_custom_activity_ids);
  ...
  if v_count <= 200 then
    select coalesce(pg_catalog.jsonb_agg(
      pg_catalog.jsonb_build_object(
        ...
        'lifecycle', pg_catalog.upper(activity.lifecycle),
        'mappingStatus', pg_catalog.upper(activity.mapping_status)
  ...
  return pg_catalog.jsonb_build_object(
    'contract', pg_catalog.jsonb_build_object(
      'name', 'PlanningActivityAdmissionChoicesV1', 'version', '1.0.0'
    ),
    'eligibleActivityCount', v_count,
...
-- The exact selected source includes only the display and eligibility facts required by Planning.
create function overlay.get_planning_activity_admission_source_v2(
  p_workspace_id uuid,
  p_profile_version_id uuid,
  p_activity_key text
)
...
begin
  ...
  select pg_catalog.jsonb_build_object(
    'contract', pg_catalog.jsonb_build_object(
      'name', 'PlanningActivityAdmissionSourceV2', 'version', '2.0.0'
    ),
...
create function api.get_learning_track_activity_admission_source_v1()
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if not found then
    return pg_catalog.jsonb_build_object(
      'contract', pg_catalog.jsonb_build_object(
        'name', 'LearningTrackActivityAdmissionSourceV1', 'version', '1.0.0'
      ),
      'state', 'NO_CURRENT_PLAN', 'capabilities', '[]'::jsonb,
      'growthPlan', null, 'learningTrack', null, 'activities', '[]'::jsonb
    );
  end if;
  ...
  if v_current_track_count = 1 then
  ...
  if v_current_track_count <> 1 then
    ...
    if v_non_archived_count >= 200 then
      ...
      v_choices := overlay.get_planning_activity_admission_choices_v1(
        v_workspace_id, v_track.profile_version_id, v_excluded_ids
      );
      if v_choices#>>'{contract,name}' <> 'PlanningActivityAdmissionChoicesV1'
         or v_choices#>>'{contract,version}' <> '1.0.0' then
        raise exception using errcode = '55000', message = 'Overlay admission choices contract is invalid';
      end if;
      ...
      if v_eligible_count > 200 then
        ...
        ), '[]'::jsonb) into v_public_activities
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'capabilities', case when v_state = 'READY'
      then pg_catalog.jsonb_build_array('admit_activity_to_learning_track') else '[]'::jsonb end,
    'growthPlan', pg_catalog.jsonb_build_object(
      'title', v_plan.title, 'lifecycle', pg_catalog.upper(v_plan.lifecycle),
      'weeklyCapacityMinutes', v_plan.weekly_capacity_minutes,
      'aggregateVersion', v_plan.aggregate_version::text
    ),
    'learningTrack', case when v_current_track_count = 1 then pg_catalog.jsonb_build_object(
      'trackKey', v_track.track_key, 'title', v_track.title,
      'lifecycle', pg_catalog.upper(v_track.lifecycle), 'priority', v_track.priority,
    ...
    'activities', v_public_activities
  );
end
$function$;

-- Private builder returns an internal identity envelope. The public preview strips it.
create function planning.build_learning_track_activity_admission_preview_v1(
  p_workspace_id uuid,
  p_activity_key text,
...
begin
  ...
  if v_activity_source#>>'{contract,name}' <> 'PlanningActivityAdmissionSourceV2'
     ...
     or (v_activity_source#>>'{customActivity,profileVersionId}')::uuid <> v_track.profile_version_id
  ...
  select coalesce(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object('code', warning)
    order by item.position), '[]'::jsonb) into v_warnings
  ...
  v_names := array[
    ...
    'planActivityLimit','canApply','blockingReasonCode','warningCount'
  ];
  v_values := array[
    'learning-track-activity-admission-preview-digest/1.0.0','1.0.0',
    pg_catalog.lower(p_workspace_id::text),'admit_activity_to_learning_track',
    'planning.add_learning_track_activity_v2',pg_catalog.lower(p_request_id::text),p_reason,
    p_expected_growth_plan_version::text,p_expected_learning_track_version::text,
    pg_catalog.lower(v_plan.growth_plan_id::text),v_plan.title,pg_catalog.upper(v_plan.lifecycle),
    v_plan.weekly_capacity_minutes::text,v_plan.aggregate_version::text,
    pg_catalog.lower(v_track.learning_track_id::text),v_track.track_key,v_track.title,
    pg_catalog.upper(v_track.lifecycle),v_track.priority::text,
    v_track.protected_minimum_minutes::text,v_track.default_session_minutes::text,
    v_track.aggregate_version::text,(v_track.aggregate_version + 1)::text,
    pg_catalog.lower(v_track.readiness_goal_id::text),pg_catalog.lower(v_track.profile_version_id::text),
    v_goal_source->>'ownerRevision',pg_catalog.lower(v_custom_activity_id::text),p_activity_key,
    v_activity_source#>>'{customActivity,title}',v_activity_source#>>'{customActivity,activityType}',
    v_activity_source#>>'{customActivity,targetCompetencyRef}','ACTIVE','ACCEPTED',
    v_activity_source->>'ownerRevision',v_candidate_key,p_estimated_minutes::text,
    coalesce(p_energy,''),v_count::text,(v_count + 1)::text,'200',
  ...
  return pg_catalog.jsonb_build_object(
    'contract', pg_catalog.jsonb_build_object(
      'name', 'LearningTrackActivityAdmissionPreviewV1', 'version', '1.0.0'
    ),
    ...
    'requestId', p_request_id, 'reason', p_reason,
    ...
    'activity', pg_catalog.jsonb_build_object(
      ...
      'candidateKey', v_candidate_key, 'estimatedMinutes', p_estimated_minutes,
      'energy', p_energy
    ),
    'constraint', pg_catalog.jsonb_build_object(
      'planActivityCountBefore', v_count, 'planActivityCountAfter', v_count + 1,
      'planActivityLimit', 200
    ),
    'canApply', v_can_apply,
    'blockingReasons', case when v_blocker is null then '[]'::jsonb
      else pg_catalog.jsonb_build_array(pg_catalog.jsonb_build_object('code', v_blocker)) end,
    'warnings', v_warnings,
    'retained', pg_catalog.jsonb_build_object(
      'activitiesAndEvidence', true, 'planSnapshots', true,
      'focusSessions', true, 'masteryAndReadiness', true
    ),
    'recalculationAfterApply', pg_catalog.jsonb_build_object(
      'projectionState', 'PENDING', 'eventChangeKind', 'TRACK_ACTIVITY_ADMITTED',
...
create function api.preview_learning_track_activity_admission_v1(
  p_activity_key text,
  p_estimated_minutes integer,
  p_energy text,
...
begin
  ...
  if p_activity_key is null or p_activity_key !~ '^activity:[a-z0-9][a-z0-9-]{1,100}$'
     ...
     or pg_catalog.char_length(p_reason) not between 1 and 500 or p_reason ~ '[[:cntrl:]]'
  ...
  v_preview := planning.build_learning_track_activity_admission_preview_v1(
    v_workspace_id, p_activity_key, p_estimated_minutes, p_energy,
    p_expected_growth_plan_version::bigint, p_expected_learning_track_version::bigint,
    p_reason, p_request_id::uuid
  );
  return v_preview - 'internal';
end
$function$;

create function api.apply_learning_track_activity_admission_v1(
  p_activity_key text,
  p_estimated_minutes integer,
  p_energy text,
  p_expected_growth_plan_version text,
...
begin
  ...
  v_idempotency_key := 'learning-track-activity-admission:v2:' || p_request_id;
  v_request_hash := extensions.digest(pg_catalog.convert_to(
    planning.frame_named_fields_v1(
      ...
      array[
        ...
        'planning.add_learning_track_activity_v2',pg_catalog.lower(v_workspace_id::text),
        p_request_id,p_activity_key,p_estimated_minutes::text,coalesce(p_energy,''),
        p_expected_growth_plan_version,p_expected_learning_track_version,p_reason,p_preview_digest
      ]
    ), 'UTF8'), 'sha256');

  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(
    v_actor_user_id::text || ':planning.add_learning_track_activity_v2:' || v_idempotency_key, 0
  ));
  select receipt.* into v_receipt
  from outbox.command_receipts as receipt
  where receipt.actor_user_id = v_actor_user_id
    and receipt.command_type = 'planning.add_learning_track_activity_v2'
  ...
  insert into outbox.command_receipts (
    ...
    v_command_id, 'planning.add_learning_track_activity_v2', 1, v_workspace_id,
    v_actor_user_id, v_idempotency_key, v_request_hash, v_correlation_id,
    p_expected_learning_track_version::bigint
  );
  ...
  set aggregate_version = aggregate_version + 1, updated_at = pg_catalog.clock_timestamp()
  where workspace_id = v_workspace_id and growth_plan_id = v_plan_id
  ...
  insert into outbox.events (
    ...
    'planning.learning_track', v_track_id,
    p_expected_learning_track_version::bigint + 1, 'user', v_actor_user_id,
    v_command_id, v_correlation_id, pg_catalog.clock_timestamp(), 'pando.database',
    pg_catalog.jsonb_build_object(
      'change_kind', 'TRACK_ACTIVITY_ADMITTED', 'growth_plan_id', v_plan_id,
      'learning_track_id', v_track_id,
      'learning_track_version', (p_expected_learning_track_version::bigint + 1)::text,
      'custom_activity_id', v_custom_activity_id, 'candidate_key', v_candidate_key
    )
  );
  get diagnostics v_affected_rows = row_count;
  if v_affected_rows <> 1 then
  ...
  v_response := pg_catalog.jsonb_build_object(
    'contract', pg_catalog.jsonb_build_object(
      'name', 'LearningTrackActivityAdmissionApplyResultV1', 'version', '1.0.0'
    ),
    ...
    'admittedActivity', pg_catalog.jsonb_build_object(
      'activityKey', p_activity_key, 'candidateKey', v_candidate_key,
      'estimatedMinutes', p_estimated_minutes, 'energy', p_energy
    ),
    'projectionState', 'PENDING', 'planningDeliveryId', v_delivery_id,
...
alter function overlay.get_planning_activity_admission_choices_v1(uuid, uuid, uuid[])
  owner to pando_phase1_api;
alter function overlay.get_planning_activity_admission_source_v2(uuid, uuid, text)
  owner to pando_phase1_api;
alter function api.get_learning_track_activity_admission_source_v1()
  owner to pando_planning_api;
alter function planning.build_learning_track_activity_admission_preview_v1(
  uuid, text, integer, text, bigint, bigint, text, uuid
) owner to pando_planning_api;
alter function api.preview_learning_track_activity_admission_v1(
  text, integer, text, text, text, text, text
) owner to pando_planning_api;
alter function api.apply_learning_track_activity_admission_v1(
...
revoke all on function
  ...
  planning.build_learning_track_activity_admission_preview_v1(
    uuid, text, integer, text, bigint, bigint, text, uuid
  ),
  api.get_learning_track_activity_admission_source_v1(),
  api.preview_learning_track_activity_admission_v1(
    text, integer, text, text, text, text, text
  ),
...
-- The released v1 schema remains for immutable historical receipts, but it is no longer a runtime
-- mutation path because it lacks exact preview and confirmation.
revoke execute on function api.add_learning_track_activity_v1(
  text, text, integer, text, text, text
) from authenticated;
revoke execute on function planning.add_learning_track_activity_impl_v1(
