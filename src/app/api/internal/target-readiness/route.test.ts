...
vi.mock("../../../../modules/targets/application/dispatch-target-readiness-projection", () => ({
  dispatchTargetReadinessProjection: mocks.dispatch,
}));
...
describe("internal Target readiness projection route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("rejects absent, malformed, or non-equal bearer secrets", async () => {
    for (const authorization of [undefined, "Basic value", `Bearer ${secret}x`]) {
      const response = await GET(
        new Request("https://pando.test/api/internal/target-readiness", {
