...
import { dispatchTargetReadinessProjection } from "../../../../modules/targets/application/dispatch-target-readiness-projection";
import {
...
async function dispatch(request: Request): Promise<NextResponse> {
  try {
    ...
    const summary = await dispatchTargetReadinessProjection(createPandoInternalProjectionClient());
