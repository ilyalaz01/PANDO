...
vi.mock("../../../../modules/planning/application/dispatch-plan-snapshot-projection", () => ({
  dispatchPlanSnapshotProjection: mocks.dispatch,
}));
...
describe("internal Planning snapshot projection route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("rejects absent, malformed, or non-equal bearer secrets", async () => {
    for (const authorization of [undefined, "Basic value", `Bearer ${secret}x`]) {
      const response = await GET(
        new Request("https://pando.test/api/internal/planning-snapshot", {
